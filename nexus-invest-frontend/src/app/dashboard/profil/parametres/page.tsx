    ...
    <div className="flex flex-col gap-6 max-w-2xl mx-auto">
          ...
          {(['current', 'new', 'confirm'] as const).map((field) => (
            <div key={field}>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                {field === 'current' ? t('settings.current_password') : field === 'new' ? t('settings.new_password') : t('settings.confirm_password')}
              </label>
      ...
      <GlassCard variant="default" padding="md">
        ...
        <div className="flex flex-col gap-3 mb-4">
          {([
            { key: 'email' as const, label: t('settings.email_notifications') },
            { key: 'sms' as const, label: t('settings.sms_notifications') },
            { key: 'push' as const, label: t('settings.push_notifications') },
          ]).map(({ key, label }) => (
      ...
      <GlassCard variant="default" padding="md">
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-700 p-3">
          <span className="text-sm font-medium text-slate-900 dark:text-white">{t('settings.theme_label')}</span>
          <GradientButton
            variant="secondary"
            size="sm"
