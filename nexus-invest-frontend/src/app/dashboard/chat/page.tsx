...
function AutoResizeTextarea({
  ...
  return (
    <textarea
      ...
      rows={1}
      className="flex-1 resize-none overflow-y-hidden rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 py-2.5 px-4 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 disabled:opacity-50 disabled:cursor-not-allowed"
      aria-label="Message"
    />
  );
}
...
export default function ChatPage() {
  ...
  if (rooms.length === 0) {
    return (
      <div className="flex flex-col gap-4">
        ...
        <EmptyState
          icon={MessageCircle}
          title={t('chat.no_rooms')}
          description={t('chat.no_rooms_desc')}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4 max-h-[calc(100vh-12rem)] overflow-hidden">
