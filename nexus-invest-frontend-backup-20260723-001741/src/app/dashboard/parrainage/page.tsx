...
import { Users, Copy, Check, Gift, TrendingUp, UserPlus, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ReferralPage() {
  ...
  const { stats, tree, loading, error } = useReferrals();
  const [copied, setCopied] = useState(false);

  const { user } = useUser();
  const referralLink = `https://nexuscoin.com/inscription?ref=${user?.referral_code ?? 'PARRAIN'}`;
